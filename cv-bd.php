...
<body>
    ...
    if (
        ...
        </p><div display:block; ></div><div style="  background:white;"><a href=""><input type="text" value="Retour à la formulaire" onclick="history.back()" style="color: white; font-family: sans-serif; border: 1px solid black; width: 165px; height: 44px; border-radius: 20px; font-size: 15px; background-color: #e6333c; margin-top:110px;"></div>';
    } else {
        $prenom = $_SESSION['prenom'];
        $nom = $_SESSION['nom'];
        $conn = mysqli_connect($servername, $username, $password, 'autorecrute');
        $sel = mysqli_select_db($conn, 'autorecrute') or die("erreur de connexion base");
        $req = mysqli_query($conn, "INSERT INTO cv (id,nom,prenom,file,titre,niveauexp,fonction,niveauetd,contrat,consultable,expprof,edcform,langues,comppers,infocomp) 
    VALUES (null,'$nom','$prenom','$file','$titre','$niveau','$fonction','$nvetude','$contrat','$consultable','$infosupp','$edecform','$langue','$comppers','$infocomp')");
        ...
        if ($req) {
            ...
            <div id=face>
    <div id=face1 >
    <p style='color:green; margin-bottom:575px; background:#d6e9c6; border:1px solid green; width:600; padding:10;'>On a bien reçu votre cv.</p>
    </div>
    <div id='face2'>
    <div class='first'>
